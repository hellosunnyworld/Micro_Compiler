...
// return the result reg
        ...
        if (target_expr ->mode == 0){
                ...
                expression = global_expr_ls + target_expr->nxt_expr_id;
...
int search_temp(int val){
        for (int i = 0; i < 8; i++){
                if (temp_reg[i] == val)
                        return i;
        }
        int addr = search_aval_temp();
        std::string pos, cmd;
        if (addr != -1){
                temp_reg[addr] = val;
                //pos = "$t" + std::to_string(addr);
                //cmd = "addi " + pos + ", $zero, " + std::to_string(val);
                //mips_code.push_back(cmd);   
        }
        return addr;
}

int search_aval_temp(){
        for (int i = 0; i < 8; i++){
                if (temp_aval[i] == 1){
                        temp_aval[i] = 0;
                        return i;
...
// store id, id = var1 op var2 (store:move pt, gen mips, add table)
        ...
        // else if (var1[1] == 't'){
        //         temp_aval[stoi(var1.substr(2,var1.size()-2))] = 1;
        // }
        ...
        // else if (var2[1] == 't'){
        //         temp_aval[stoi(var2.substr(2,var2.size()-2))] = 1;
        // }
        
        std::string ops(op);
        cmd = ops + " ";
        if (strcmp(id, "temp!") && (search_table(id) != std::string(""))){
                addr = search_table(id);
                cmd += addr + ", "+ var1 + ", " + var2;
                mips_code.push_back(cmd);                
        }
        else if (strcmp(id, "temp!") && (crt_saved_pt - saved_reg < 8)){
                ...
                int pos = 0;
                if ((strcmp(op, "addi") == 0) && (var1 == std::string("$zero")))
                        pos = search_temp(stoi(var2));
                else
                        pos = search_aval_temp();
                ...
                mips_code.push_back(cmd);
                addr = std::to_string(4*(crt_mem_pt - mem)) + "($sp)";
